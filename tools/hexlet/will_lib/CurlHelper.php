...
use Exception;
use hexlet\will_lib\exceptions\CurlHelperException;
 ...
 *      @see CurlHelper::curl_helper()
 *
 *   added helper function @see CurlHelper::set_debug_mode()
 * </p>
 */
class CurlHelper {
	protected $ch = null;
	 ...
	 * @throws CurlHelperException
	 */
	function upload_file_and_data( string $url, array $file_names, array $fields,$format='text') {
// data fields for POST request
	 ...
	 * @param integer &$http_code , will be set to the integer return code of the server. Its only an output variable
	 *
	...
	function curl_helper(
		string $url, int &$http_code, $fields = null, $b_post = true, $format = 'json',
			...
			try {
				switch ( $format ) {
					...
					case 'json_force_array':
						$data_out = CurlHelper::json_from_string( $curl_output,true,true );
						break;
					case 'xml':
