...
use Exception;
use hexlet\will_lib\exceptions\SQLDataException;
...
class MYDB
{
    ...
    {
        $ee = new MYDBEnumValues($table, $column, $this->mysqli);
    ...
    public function __construct( ?object $mysqli, array $db_setup = null, $bIgnoreAardvark = false)
    {
        ...
        if (is_null($mysqli)) {
            ...
            if (!$bIgnoreAardvark) { //only if deliberate should the program not exit if more than one connection open
                if ($this->AnExistCount > 1) {
                    try {
	                    ...
	                    throw new exceptions\SQLException("++>exists greater than one [$what]", $this->AnExistCount);
            ...
            if (!($mysqli instanceof mysqli)) {
	            ...
	            throw new exceptions\SQLException("the mysqli passed in is not who you think it is", $out);
            }
            $this->AnExistCount = MYDB::$aardvark;
     ...
     *   But will only do the work when this is called same number of times as @return void
     *@throws SQLException if beginTransaction was not called ahead of time
     * @see MYDB::beginTransaction() was before
     */
    public function commit()
    {
        ...
        if ($this->mysqli->transactionCount < 0) {
	        throw new exceptions\SQLException('Commit in mydb is misaligned', $this->mysqli->transactionCount);
        }
        if ($this->mysqli->transactionCount > 0) {
            return;
        }
     ...
     * @throws \hexlet\will_lib\exceptions\SQLException
    ...
    /**
     * Can get the data after using @return array|null
    ...
    public function fetchThrowIfNull()
    {
        ...
        if (!$res) {
            throw new exceptions\SQLDataException("no_data");
        }
        return $res;
    }
     ...
     * @throws \hexlet\will_lib\exceptions\SQLException  if there is any sql error
     */
    ...
    public function execSQL($sql, $params=null, $close=MYDB::RESULT_SET, $lookupKey = null)
    {
        ...
        if (! (is_array($params) || is_null($params ) ) ) {
	        throw new exceptions\SQLException("Params need to be an array or null");
        }
        $parameters = [];
        $row = [];
        $mysqli = $this->mysqli;
        ...
        if (is_object($sql) && get_class($sql) == 'mysqli_stmt') {
	        ...
	        if ( strpos($lookupKey,'@sey@') === false ) {
		        throw new exceptions\SQLException("Lookup key does not have @sey@ in it somewhere");
	        }
        ...
        try {
	        ...
	        if ($params && is_array($params)) {
		        for($i = 1; $i < count($params); $i++) {
			        $params[$i] = JsonHelper::toStringAgnostic($params[$i],true,true);
		        }
	        }
        } catch ( Exception $e ) {
	        throw new exceptions\SQLException( $e->getMessage() . " : Index was $i");
        }
    ...
    public function update( string $table, $id, array $fields, $pk_name = 'id')
    {
	    ...
	    /**
	     * @var MYDBValue|string $value
	     */
        foreach ($fields as $name => $value) {

	        if (is_object($value) && is_a($value,'gokabam_lib\MYDB_Value')) {
    ...
    {
	    if ( ! is_array($fields) || empty($fields)) { throw new exceptions\SQLException("Insert Function in database class needs frields to be a populated array");}
	 ...
	 */
    protected static function getMySqliDatabase( array $configs )
    {
        ...
        $mysqli = mysqli_connect($sqladdress, $MySQLUsername, $MySQLPassword, $dbname,$port);
        if (!$mysqli) {
            throw new exceptions\SQLException(sprintf("Connect failed:<br> %s\n", mysqli_connect_error()));
        }
	 ...
	 * @param string $sql additional information, usually here the sql string that caused the issue
	 *
    ...
    public static function throwSQLError( $mysqli, string $sql )
    {
        ...
        throw new exceptions\SQLException($help);
    }
    ...
    {
        if ($msg2) {
            throw new exceptions\SQLException(sprintf("%s:<br> %s<br>    error:<br>%s<br>", $msg1, $msg2, mysqli_error($mysqli)));
        } else {
            throw new exceptions\SQLException(sprintf("%s:<br>error:<br>%s<br>", $msg1, mysqli_error($mysqli)));
        }
    }
...
//
    ...
    {
        if ($msg2) {
            throw new exceptions\SQLException(sprintf("error:<br>\n%s<br>\n%s<br>\n%s<br>", $msg1, mysqli_stmt_error($stmt),$msg2));
        } else {
            throw new exceptions\SQLException(sprintf("%s:<br>error:<br>%s<br>", $msg1, mysqli_stmt_error($stmt)));
        }
     ...
     *
     * @return void
     * @throws \hexlet\will_lib\exceptions\SQLException if something goes wrong
     */
    public static function executeStatement($state)
    {
	    if ( (!isset($state)) || (empty($state))) {
            throw new exceptions\SQLException("statement was null or empty");
        }
    ...
    public static function staticExecute( string $query, $mysqli ) {
        if ((!isset($query)) || (empty($query))) {
            throw new exceptions\SQLException("sql was null or empty");
        }

        $res = mysqli_query($mysqli, $query);
        if ($res != true) {
            throw new exceptions\SQLException(sprintf("Could not run execute:<br> %s<br>    sql:<br>%s<br>", mysqli_error($mysqli), $query));
        }
        return $res;
    }
    ...
    public static function cleanNumber($number)
    {
        ...
        if (!$what) {
	        throw new exceptions\SQLDataException("not a whole number: $number" );
        }
    ...
    public static function sanitizeString( string $s, $b_strip_tags = false, $mysqli = null)
    {
        $s = strval($s);
        if (empty($s)) {
            $s = '';
    ...
    public static function timestampToUTCString($ts) {
       // 'YYYY-MM-DD HH:MM:SS'
        ...
        if (!$what) {
	        throw  new exceptions\SQLDataException("$ts cannot be converted to a date time");
        }
        return $what;
    }
	 ...
	 */
    public  function arrayToQuotedString( array $arr) {
        $cloned = [];
        foreach ($arr as $a) {
	 ...
	 * @throws SQLException
	 */
	public function dropTriggersLike( string $trigger_like_this ) {
		$res = $this->execSQL('SELECT database() AS the_db;');
		$schema = $res[0]->the_db;
	...
	public function execute_nested_sql_files( string $folder_path ) {
		try {
			...
			if (!$dir) {
				throw new exceptions\SQLException("Cannot Find the folder path $folder_path");
			}
