...
use Exception;
use hexlet\will_lib\exceptions\JsonException;
use hexlet\will_lib\exceptions\LibConException;
use LogicException;

/**
 * Class LibCon (LIB-rary for CON-version)
...
class LibCon {
	 ...
	 * @param string|null $detail <p>
	 *  one of the values in the  $subtype
	 *  NULL OR one of the values in the  $detail
	 ...
	 * @since 0.2.2
	 */
	public function __construct( string $format,$subtype = null,$detail = null) {

		if (empty($format)) {
			throw new exceptions\LibConException("Format missing in");
		}
		$format = strtolower($format);
		if (!in_array($format, self::FORMATS)) {
			throw new exceptions\LibConException("Format given was [$format], but not a format ");
		}
		...
		if ($subtype &&
			...
			throw new exceptions\LibConException("SubType given was [$subtype], but not a subtypes ");
		}
		...
		if ( $subtype && in_array( $subtype, self::BOOL_TEXT_SUBTYPES ) ) {
			...
			if ( empty( $detail ) ) {
				...
				if ( ! $b_found ) {
					throw new exceptions\LibConException( "Detail given was [$detail],  not found by case insensitive search in the php timezone list. see http://php.net/manual/en/timezones.php" );
				}
			}
			$detail = strtolower($detail);
			if (in_array($subtype,self::ONLY_UTC_TIMEZONE)) {
				if (($detail !== 'utc')  ) {
					throw new exceptions\LibConException( "Date Time type $subtype is always utc");
				}
			}
		...
		if ($subtype && (!$b_found)) {
			throw new exceptions\LibConException( "Could not find [$subtype] in the accepted subtype list of " . implode('|',self::SUBTYPES_SINGLE_ARRAY));
		}
		...
		$b_never = static::usually_unused_goes_here(static::TYPE_ALIAS,static::get_formats(),static::get_details_for_subformat('none'),static::get_subtype_for_format('none'));
		if ($b_never) {
			static::test();
		}
	}
	...
	protected static function usually_unused_goes_here(...$params) {
	 ...
	 */
	public static function parseFormatString( string $format_string ) {
		...
		if(empty($format)) {
			throw new exceptions\LibConException("Format string malformed, does not have a recognizable format. Got [$format_word] from $format_string");
		}

		if ($subformat_word && !$subformat) {
			throw new exceptions\LibConException("Subformat [$subformat_word] could not be found for [$format_word] ");
		}
	 ...
	 * @since 0.2.2
	 */
	public static function get_subtype_for_format( string $format) {
		$ret = [];
	 ...
	 * @since 0.2.2
	 */
	public static function get_details_for_subformat( string $subformat) {
	 ...
	 *
	 * @param null|bool|integer|float|string|array|object $what
	 ...
	 * @since 0.2.2
	 */
	public function convert( $what, LibCon $to = null) {
		/*
	...
	protected static function convert_var_to_string_array($what) {
	...
	protected static function convert_var_to_int_array($what) {
	...
	protected static function convert_var_to_json_array($what) {
	...
	protected static function convert_var_to_float_array($what) {
	...
	protected static function convert_var_to_object_array($what) {
	 ...
	 */
	protected static function convert_to_object($what) {
		if (is_null($what)) {return $what;} //nulls stay null
		if (empty($what) && (!(is_numeric($what) || is_bool($what)))) {
			...
			if (JsonHelper::isJson($what)) {
				try {
					...
					throw new exceptions\LibConException( "Cannot convert json string [$what] to object " . $bad_json->getMessage());
				}
			} else {
				throw new exceptions\LibConException("Canot use string as json. Its not json\n [$what]: ");
			}
			...
			throw new exceptions\LibConException("Cannot convert [$bad_type] to object. Its not a json string, object, empty, or array " );
		}
		return $cast;
	}
	/**
	 ...
	 * @throws JsonException
	 */
	protected static function convert_to_json($what) {
		if (is_string($what)) {
			...
			try {
				...
				throw new exceptions\LibConException( "Canot convert value to json: " . $bad_json->getMessage());
			}
		} else {
			throw new exceptions\LibConException( "Cannot covert [$what] to json. The data type is " . gettype($what));
		}
		return $cast;
	}
	 ...
	 * @throws JsonException
	 */
	protected static function convert_to_float($what) {
		if (is_null($what)) {return $what;} //nulls stay null
		if (is_numeric($what) || is_bool($what)) {
			...
			if (is_numeric($test)) {
				...
				throw  new exceptions\LibConException("$class_name is an object whose string value [$test] is not numeric so cannot convert to unix timestamp float");
			}
		} elseif (is_string($what)) {
			...
			} catch ( Exception $bad_dt ) {
				throw  new exceptions\LibConException("[$what] is not a known date time string so cannot convert to float ");
			}
			...
			throw  new exceptions\LibConException("[$val] is not numeric so cannot convert to float");
		}
		return $cast;
	}
	/**
	 ...
	 * @throws JsonException
	 */
	protected static function convert_to_text($what) {
		if (is_null($what)) {return $what;} //nulls stay null
		if (is_object($what)) {
			if ( method_exists( $what, '__toString' ) ) {
	 ...
	 * @throws JsonException
	 */
	protected static function convert_to_int($what) {
		if (is_numeric($what) || is_bool($what)) {
			$cast =  intval($what);
		}
		elseif (empty($what)) {
		...
		elseif (is_object($what)) {
			...
			if (is_numeric($test)) {
				...
				$class_name = get_class( $what );
				throw  new exceptions\LibConException("[$class_name] is an object whose string value [$test] is not numeric so cannot convert to integer");
			}
			...
			throw  new exceptions\LibConException("[$val] is not numeric so cannot convert to integer");
		}
		return $cast;
	}
	/**
	 ...
	 * @throws JsonException
	 */
	protected static function convert_to_array($what) {
		if (empty($what) && (!(is_numeric($what) || is_bool($what)))) {
			$cast = [];  //null already processed, if its empty other than null, a number or boolean
		} elseif (is_string($what)) {
			if (JsonHelper::isJson($what)) {
				try {
					$cast = JsonHelper::fromString( $what, true, true );
				} catch (JsonException $bad_json) {
					//should not get here, ever, since passed is_json
					throw new exceptions\LibConException("Cannot convert json string [$what] to array ". $bad_json->getMessage());
				}
			} else {
				$cast = [$what];
			}
		...
		elseif (is_numeric($what) || is_bool($what)) {
			...
			throw new exceptions\LibConException("Cannot convert  to array. Its not empty, or a string, object, array,numeric,boolean or empty ". gettype($what));
		}
		return $cast;
	}
	/**
	 ...
	 *
	 */
	protected static function convert_var_to_carbon($what,$timezone) {

		try {
			if ($what === false || $what === true) {
				throw new exceptions\LibConException("Cannot process boolean to DateTime");
			}
			...
			if ( is_numeric( $what ) ) {
				...
				if (is_object($what) && ($what instanceof CarbonInterface ) ){
					$start = $what->copy();
				}
				elseif (is_object($what) && ($what instanceof DateTimeInterface )) {
					...
					if ( is_object( $what ) ) {
						if ( method_exists( $what, '__toString' ) ) {
							...
							$class_name = get_class( $what );
							throw  new exceptions\LibConException( "[$class_name] is an object which does not override __toString, so cannot convert to string to decide a date" );
						}

					} else if ( is_array( $what ) ) {
						$huh = print_r( $what, true );
						throw  new exceptions\LibConException( "$huh is an array , so cannot convert to string to decide a date" );
					} else {
						$test = (string) $what;
					}
					if ( empty( $test)) {
						throw new exceptions\LibConException("Cannot create a date time from an empty string");
					}
			...
			return $start;
		} catch (exceptions\LibConException $lib) {
			throw $lib;
		} catch ( Exception $e) {
			throw new exceptions\LibConException("Could not parse datetime string",0,$e);
		}
	}
	...
	protected static function convert_var_to_subtype( $what, string $subtype, ?string $detail ) {
		...
		switch ($subtype) {
			...
			default: {
				throw new exceptions\LibConException("Cannot cast subtype, do not recognize [$subtype], it needs to be one of ". implode('|',self::SUBTYPES_SINGLE_ARRAY));
			}
		}
	}
	 ...
	 * @since 0.2.2
	 */
	protected static function convert_var_to_type( $what, string $type ) {
			...
			case self::BINARY_FORMAT : {
				if (is_object($what)) {
					if ( method_exists( $what, '__toString' ) ) {
						...
						$class_name = get_class($what);
						throw  new exceptions\LibConException("[$class_name] is an object which does not implement the __toString override function. It has no text conversion. Use the Json Case instead");
					}
				} else if (is_array($what)) {
	 ...
	 */
	public static function test($error_level = 0) {
		$types_to_test = [
							'bool',
							'tri',
							'int',
